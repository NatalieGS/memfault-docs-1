...
All values will be shown in HRT on timeline. Entries will only be created in
the metric report for each requested aggregation.
...
Even if no aggregation types are reuqested, all state transitions will be
